						...
						<table class="tb_02" style="TABLE-layout: fixed" width="100%" border="0" cellspacing="0" cellpadding="0">
							...
							<c:forEach items="${list }" var="item" varStatus="status">
								<tr id="${item.contractor_id }">
									...
									<td style="text-align: right; padding-right: 14px;display:none;">${item.memo}</td>
								</tr>
							</c:forEach>
					...
					<div id="contractorInput">
						...
						<table width="580" cellpadding="6" cellspacing="0" border="0" class="tb_03">
							...
							<tr>
								...
								<td colspan="3" width="25%"><input name="phone" id="phone" type="text" size="50" value="" maxlength="430" class="input01 essence" /></td>

							</tr>
							<tr>
								<td class="td_title" width="25%"><img src="${wc}/eams/images/eams/common/icon_bul2.gif" /> 업체 사용여부</td>
							...
							<tr>
								<td class="td_title" width="25%"><img src="${wc}/eams/images/eams/common/icon_bul2.gif" /> 업체 비고</td>
								<td colspan="3" width="75%"><input name="memo" id="memo" type="text" size="60" value="" maxlength="1030" class="input01 essence" />
								</td>
...
<!--
	...
	$(document).ready(function() {
		...
		$(document).delegate("div.tb_list > table > tbody > tr", 'click', function() {
			...
			$('#memo').val($(this).children('td:eq(4)').text());
		...
		$("#saveForm").click(function() {
		 ...
		 $("#list").jqGrid({
		        ...
		        url: "${wc}/xo/listJson.do",
		        datatype: "json",
		        mtype: "POST",
		        colNames: ["아이디", "업체명", "사업자번호", "연락처"],
		        colModel: [
		            { name: "contractor_id", index : 'contractor_id', width: 105 },
		            { name: "contractor_name", index : 'contractor_name', width: 190 },
		            { name: "registration_number", index : 'registration_number', width: 180, align: "right" },
		            { name: "phone", index : 'phone', width: 180, align: "right" }
		        ],
		        pager: "#pager", 
		        header:false,
		        rowNum: 10,
		        rowList: [ 10, 20, 30],		        
				autowidth : true, //테이블의 가로길이 자동지정
				height : 'auto',   // pixel or '100%'  or 'auto'
				altRows : true,  //row의 색 자동지정
		        sortname: "contractor_name",
		        sortorder: "desc",
		        viewrecords: true,
		        gridview: true,
		        autoencode: true,
		        scrollOffset: 20,   // 스크롤바크기
				jsonReader : {
					root: "rows", 
					page: "page", 
					total: "total", 
				...
				onSelectRow : function(rowid, iRow, iCol, e){ 
					var pss = jQuery(this).jqGrid('getRowData', rowid);
					var namex = pss['contractor_id'];
					alert(namex);
					$('#contractor_id').val( pss['contractor_id'] );

					$('#contractor_id_span').attr('class', 'update_contr');
					$('#contractor_id_span').text('수정');

					$('#contractor_name').val( pss['contractor_name'] );
					$('#registration_number').val( pss['registration_number'] );
					$('#phone').val( pss['phone'] );
					// Y N 값에 따라 래디오버튼의 값을 셋팅
					//var use = pss['use'] $(this).children('td:eq(3)').text();
					//$('input:radio[name=use_yn]:input[value=' + use + ']').attr("checked", true);
			...
			$('div.category').qtip({
				   content: '정보화사업시스템 계약업체 관리 페이지임. \
						   <br>EA 관리자 권한이 있는 경우에 신규, 수정, \
						   <br>삭제가 가능함. \
					',
				   show: 'mouseover',
				   hide: 'mouseout',
